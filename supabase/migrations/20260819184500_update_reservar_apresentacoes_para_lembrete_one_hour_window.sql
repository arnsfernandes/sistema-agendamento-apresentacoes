-- Migration: Update reservar_apresentacoes_para_lembrete to restrict window between 55 and 60 minutes
-- Date: 2026-08-19 18:45:00
...
BEGIN
  ...
  WITH selecionados AS (
    ...
    WHERE a.lembrete_enviado_em IS NULL
      ...
      AND (a.data + a.horario) AT TIME ZONE 'America/Sao_Paulo' > now() + interval '55 minutes'
      AND (a.data + a.horario) AT TIME ZONE 'America/Sao_Paulo' <= now() + interval '60 minutes'
