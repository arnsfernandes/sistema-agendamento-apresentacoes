...
import { reactivateParticipant, BusinessRuleError } from '../_shared/scheduling.ts'
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 3. Call shared reactivation logic
    const result = await reactivateParticipant(
      supabaseAdmin,
      userId,
      googleIntegracaoId,
      participationId
    ...
    return jsonResponse({ error: error.message || 'Ocorreu um erro interno ao reativar a participação.' }, 500)
  }
})
