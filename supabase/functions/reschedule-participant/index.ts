...
import { rescheduleParticipant, BusinessRuleError } from '../_shared/scheduling.ts'
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Parse request body
    const { participantId, fromMeetingId, toMeetingId } = await req.json()

    if (!participantId || !fromMeetingId || !toMeetingId) {
      return jsonResponse({ error: 'Dados de remarcação incompletos.' }, 400)
    }
    ...
    // 3. Call shared rescheduling rules
    const result = await rescheduleParticipant(
      supabaseAdmin,
      userId,
      googleIntegracaoId,
      participantId,
    ...
    return jsonResponse({ error: error.message || 'Ocorreu um erro interno ao remarcar o participante.' }, 500)
  }
})
