-- Migration: Update reservar_apresentacoes_para_lembrete to include 1 hour time filter
-- Date: 2026-08-19 15:02:00
...
BEGIN
  ...
  WITH selecionados AS (
    ...
    WHERE a.lembrete_enviado_em IS NULL
      ...
      AND (a.data + a.horario) AT TIME ZONE 'America/Sao_Paulo' > now()
      AND (a.data + a.horario) AT TIME ZONE 'America/Sao_Paulo' <= now() + interval '1 hour'
