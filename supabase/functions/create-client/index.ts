import { createClient as supabaseCreateClient } from 'npm:@supabase/supabase-js@2'
import { createClient, BusinessRuleError } from '../_shared/scheduling.ts'
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const supabaseUser = supabaseCreateClient(supabaseUrl, anonKey)
    ...
    // Parse request body
    const { nome, telefone, agencia } = await req.json()

    const supabaseAdmin = supabaseCreateClient(supabaseUrl, serviceRoleKey)
    ...
    // 3. Call shared creation rules
    const client = await createClient(
      supabaseAdmin,
      userId,
      googleIntegracaoId,
      { nome, telefone, agencia }
    )
    ...
    return jsonResponse({ error: error.message || 'Ocorreu um erro interno ao cadastrar o cliente.' }, 500)
  }
})
