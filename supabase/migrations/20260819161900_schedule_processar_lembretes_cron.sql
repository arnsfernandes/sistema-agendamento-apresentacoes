...
-- Schedule the processar-lembretes Edge Function every 5 minutes using pg_cron and pg_net
SELECT cron.schedule(
  ...
  SELECT net.http_post(
    ...
    headers := jsonb_build_object(
      ...
      'x-cron-secret', (SELECT secret FROM vault.decrypted_secrets WHERE name = 'lembretes_cron_secret' LIMIT 1)
    )
  );
  $$
);
